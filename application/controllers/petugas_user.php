...
class petugas_user extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('petugas/vuser',$data);
    }
    public function user_byid()
    {
        ...
        $data['quser'] = $this->muser->get_byid($NIP); //query model semua Data
    ...
    public function form(){
        ...
        if ($mau_ke == "add") {
            ...
            $data['aksi'] = 'aksi_add';
            $this->load->view('petugas/vformuser',$data);
        } else if ($mau_ke == "edit") {
         ...
         if ($PASSWORD==$PASSWORD2 ) {
            ...
            redirect(base_url().'petugas_user');
         } else {
            ...
            redirect(base_url().'petugas_user/form/add');
         }
            ...
            if ($PASSWORD_LAMA==$PASSWORD_ACC) {
                ...
                redirect(base_url().'petugas_user/user_byid');
                }else{
                    $this->session->set_flashdata("pesan", "<div class=\"alert alert-danger\" id=\"alert\"><i class=\"glyphicon glyphicon-ok\"></i> Password tidak cocok! </div>");
                    redirect(base_url().'petugas_user/form/edit/'.$NIP);
                }
            } else {
