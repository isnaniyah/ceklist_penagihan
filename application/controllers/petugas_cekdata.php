...
class Petugas_cekdata extends CI_Controller {
	...
	public function index($id)
	{
		...
		$this->load->view('petugas/vcekdata',$data);
	}
    ...
    public function form(){
		...
		if ($mau_ke == "add") {
		    ...
		    $data['aksi'] = 'aksi_add';
            $this->load->view('petugas/vformcekdata',$data);
		} else if ($mau_ke == "edit") {
            ...
            if ($cek_no_pembayaran>0) {
                ...
                redirect(base_url().'petugas_cekdata/form/add/'.base64_encode($ID_KONTRAK));
                ...
                redirect(base_url().'petugas_kontrak/detail/'.$ID_KONTRAK2);
            }
            ...
            if ($cek_no_pembayaran>1) {
                $this->session->set_flashdata("pesan", "<div class=\"alert alert-danger\" id=\"alert\"><i class=\"glyphicon glyphicon-ok\"></i> Penagihan ke-".$NO_PEMBAYARAN." sudah ada! </div>");
                redirect(base_url().'petugas_cekdata/form/edit/'.$ID_DATA_CEKLIST);
            }else if ($TGL_KONTRAK > $TGL_ACC1) {
                $this->session->set_flashdata("pesan", "<div class=\"alert alert-danger\" id=\"alert\"><i class=\"glyphicon glyphicon-ok\"></i> Tanggal acc sebelum tanggal kontrak! </div>");
                redirect(base_url().'petugas_cekdata/form/edit/'.$ID_DATA_CEKLIST);
            }else{
    ...
    public function detail($id){
		...
		$this->load->view('petugas/vdetcekdata',$data);
        $this->load->view('petugas/vceklist',$data2);
    }
    public function hapus($gid,$ID_KONTRAK){
        ...
        redirect(base_url().'petugas_kontrak/detail/'.$ID_KONTRAK);
	}
}