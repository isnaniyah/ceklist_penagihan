...
class Petugas_kontrak extends CI_Controller {

	/*****
     | CRUD Data
     | controller Data view, create, update, delete
     | by g2tech
	 */
	...
	{
	    $data['title'] = 'Daftar Kontrak'; //judul title
	    $data['qkontrak'] = $this->mkontrak->get_allkontrak(); //query model semua Data
		$this->load->view('petugas/vkontrak',$data);
    ...
    public function form(){
		...
		if ($mau_ke == "add") {
		    ...
		    $data['aksi'] = 'aksi_add';
            $this->load->view('petugas/vformkontrak',$data);
		} else if ($mau_ke == "edit") {
            ...
            if ($cek>0) {
                ...
                redirect(base_url().'petugas_kontrak/form/add');
            }else{
                $data = array(
                ...
                redirect(base_url().'petugas_syarat/detail/'.base64_encode($ID_KONTRAK));
            }
          ...
          $data = array(
			...
			redirect(base_url().'petugas_kontrak');
		}
    }
    ...
    public function detail($ID_KONTRAK){
        ...
        $this->load->view('petugas/vdetkontrak',$data);
        $this->load->view('petugas/vcekdata',$data2);
